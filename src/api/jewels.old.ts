...
app.post('/', requireTier('coherent'), async (c) => {
  const user = c.get('user') as Zoku;
  const db = new DB(c.env.DB);
  const body = await validateBody(c, createJewelSchema);
...
app.patch('/:id', requireTier('coherent'), async (c) => {
  ...
  const db = new DB(c.env.DB);
  const id = c.req.param('id');
  const body = await validateBody(c, updateJewelSchema);
  ...
  if (jewel.owner_id !== user.id && user.access_tier !== 'prime') {
    throw new ForbiddenError('Can only update your own jewels');
  }
...
app.delete('/:id', requireTier('coherent'), async (c) => {
  ...
  if (jewel.owner_id !== user.id && user.access_tier !== 'prime') {
    throw new ForbiddenError('Can only delete your own jewels');
  }

  // Check if jewel is in use
  const usage = await db.getJewelUsage(id);
  if (usage.length > 0) {
    throw new ValidationError(`Cannot delete jewel: used by ${usage.length} source(s)`, { usage });
  }
