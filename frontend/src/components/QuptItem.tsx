...
export default function QuptItem({ qupt, showEntanglementName = false, onDelete }: QuptItemProps) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    setDeleting(true)
    try {
      await api.deleteQupt(qupt.id)
      addNotification('success', 'Activity deleted')
      onDelete?.()
    } catch (error) {
      console.error('Failed to delete qupt:', error)
      addNotification('error', error instanceof Error ? error.message : 'Failed to delete activity')
    } finally {
      setDeleting(false)
    }
  }
